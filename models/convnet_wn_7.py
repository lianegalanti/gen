...
class ConvNet_WN_7(nn.Module):
    def __init__(self, settings):
        super(ConvNet_WN_7, self).__init__()
        ...
        layers = [self.conv1, self.conv2, self.conv3, self.conv4,self.conv5, self.conv6, self.conv7]
        self.layers = nn.Sequential(*layers)
        self.fc = nn.Linear(self.width * 21 * 21, settings.num_output_classes)
        
        self.all_layers = [self.conv1, self.conv2, self.conv3, self.conv4,self.conv5, self.conv6, self.conv7, self.fc]
        self.degs = [2, 2, 2, 2,2,2,2]
        self.depth = 8
        self.conv_depth = 7
        self.fc_depth = 1
...
def convnet_wn_7(settings):
    return ConvNet_WN_7(settings)