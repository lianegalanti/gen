...
class ConvNet_WN_8(nn.Module):
    def __init__(self, settings):
        super(ConvNet_WN_8, self).__init__()
        ...
        self.conv7 = weight_norm(nn.Conv2d(self.width, self.width, kernel_size=2), dim=None)
        self.conv8 = weight_norm(nn.Conv2d(self.width, self.width, kernel_size=2), dim=None)

        layers = [self.conv1, self.conv2, self.conv3, self.conv4,self.conv5, self.conv6, self.conv7, self.conv8]
        self.layers = nn.Sequential(*layers)
        self.fc = nn.Linear(self.width * 20 * 20, settings.num_output_classes)
        
        self.all_layers = [self.conv1, self.conv2, self.conv3, self.conv4,self.conv5, self.conv6, self.conv7, self.conv8, self.fc]
        self.degs = [2, 2, 2, 2,2,2,2,2]
        self.depth = 9
        self.conv_depth = 8
        self.fc_depth = 1
...
def convnet_wn_8(settings):
    return ConvNet_WN_8(settings)